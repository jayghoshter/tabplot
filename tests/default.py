...
class TestExamplePlot(unittest.TestCase):
    ...
    def create_test_files(self):
        ...
        np.savetxt(
            self.resource_dir / "test.csv", np.stack([x, y], axis=1), delimiter=","
        )
    ...
    def test_files_spec_in_read(self):
        ...
        reference_filename = "line_plot_basic.png"
        plot = Plot(destdir=self.output_dir, figsize=(4.0, 4.0), show_legend=False)
        plot.read(files=[str(self.resource_dir / "test.csv")]).draw().save(
            output_filename
        )
