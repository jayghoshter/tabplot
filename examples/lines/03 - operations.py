...
plot.read().multiscale(x=[1,1.2,1], y=[1,1,1.2]).draw().save('operations-1.pdf').close()


plot = Plot(
    files = ['./line_a.tsv'],
).read().normalize_xy().trim('x<0.4').draw().save('operations-2.pdf')
...
plot = Plot(
    files = ['./line_a.tsv', './line_b.tsv'],
    labels = ['A', 'B']
).read().normalize_xy().multiscale(y=[1.0,2.0]).draw()
plot.fit_lines(labels=['one', 'two'], color = ['magenta', 'green'], linewidth=0.5)
plot.hlines([0.6,0.8], colors=['gray','r'], alpha=0.6, zorder=-1, ls='dashdot', lw=0.5)
